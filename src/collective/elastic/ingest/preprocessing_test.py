# -*- coding: utf-8 -*-
from .ingest import preprocessing
...
def test_add_additional_field__main_section_empty():
    full_schema = {}
    preprocessing.add_additional_field(full_schema, "test", {1: 2})
    assert full_schema == {"additional": {"test": [{1: 2}]}}


def test_add_additional_field__main_section_exists():
    full_schema = {"additional": {}}
    preprocessing.add_additional_field(full_schema, "test", {1: 2})
    assert full_schema == {"additional": {"test": [{1: 2}]}}


def test_add_additional_field__main_and_sub_section_exists():
...
def test_rid_leverage__None_schema():
    ...
    assert full_schema is None


def test_rid_leverage__():
    content = {"@components": {"catalog_rid": 1}}
    full_schema = {}
    preprocessing._rid_leverage(content, full_schema, "test")
    assert content == {"@components": {"catalog_rid": 1}, "rid": 1}
...
def test_remove_entry():
    content = {"delete_me": "Bee", "keep_me": "Bumblebee"}
    preprocessing._remove_entry(content, {}, "delete_me")
    assert content == {"keep_me": "Bumblebee"}


def test_remove_entry__non_existent():
    content = {"keep_me": "Bumblebee"}
    preprocessing._remove_entry(content, {}, "delete_me")
    assert content == {"keep_me": "Bumblebee"}
